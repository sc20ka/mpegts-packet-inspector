            ...
            <Descriptions bordered column={1} size="small">
              ...
              {isFieldVisible(packet, 'adaptationField.extension') && packet.adaptationField.extension && (
                <Descriptions.Item label="Adaptation Field Extension">
                  <Space direction="vertical">
                    <Text>Length: <Text code>{packet.adaptationField.extension.length}</Text></Text>

                    {packet.adaptationField.extension.ltw && (
                      <Text>
                        LTW Valid: <Tag color={packet.adaptationField.extension.ltw.validFlag ? 'blue' : 'default'}>
                          {packet.adaptationField.extension.ltw.validFlag ? 'Yes' : 'No'}
                        </Tag>
                        , Offset: <Text code>{packet.adaptationField.extension.ltw.offset}</Text>
                      </Text>
                    )}

                    {packet.adaptationField.extension.piecewiseRate && (
                      <Text>
                        Piecewise Rate: <Text code>{packet.adaptationField.extension.piecewiseRate}</Text>
                      </Text>
                    )}

                    {packet.adaptationField.extension.seamlessSplice && (
                      <Text>
                        Seamless Splice Type: <Text code>{packet.adaptationField.extension.seamlessSplice.spliceType}</Text>
                      </Text>
                    )}
                  </Space>
                </Descriptions.Item>
